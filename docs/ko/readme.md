...
| POST | `/mcp` | MCP 서버 — `get_kiro_credits` + `list_kiro_models` 툴 |
...
`POST /mcp`는 두 개의 읽기 전용 툴을 노출하는 무상태(stateless) MCP
서버(JSON-RPC 2.0)입니다. 두 툴 모두 호출자 본인의 `ksk_…` 키를 요청 헤더로
인증하며, 서버에 자격증명을 저장하지 않습니다.

- **`get_kiro_credits`** — 현재 청구 기간의 사용량/한도/잔여 크레딧, 구독 플랜,
  다음 초기화일.
- **`list_kiro_models`** — 호출자가 사용 가능한 모델 목록(Kiro 관리 엔드포인트에서
  실시간 조회, `GET /v1/models`와 동일한 카탈로그)을 각 에이전트 형식으로 반환.
  선택 인자 `format`: `"openai"`(OpenAI `/v1/models` 형식),
  `"anthropic"`(Anthropic `/v1/models` 형식), `"both"`(기본값). 그에 따라
  `structuredContent`에 `openai` 및/또는 `anthropic` 키가 담깁니다.
